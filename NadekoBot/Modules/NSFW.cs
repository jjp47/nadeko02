    ...
    internal class NSFW : DiscordModule
    {
        ...
        public override void Install(ModuleManager manager)
        {
            manager.CreateCommands("", cgb =>
            {
                ...
                cgb.CreateCommand(Prefix + "loli")
                .Description("Shows a random image from lolibooru with a given tag. Tag is optional but preffered. (multiple tags are appended with +)\nUsage: ~loli yuri+kissing")
                .Parameter("tag", ParameterType.Unparsed)
                .Do(async e =>
                {
              var tag = e.GetArg("tag")?.Trim() ?? "";
              await e.Channel.SendMessage(await SearchHelper.GetLoliImageLink(tag));
                    });
