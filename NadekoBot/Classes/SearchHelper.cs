...
namespace NadekoBot.Classes
{
    ...
    public static class SearchHelper
    {
        ...
        public static async Task<string> GetLoliImageLink(string tag)
        {
            var rng = new Random();
            var url =
        $"http://lolibooru.moe/post/index.xml?page=dapi&s=post&q=index&limit=100&tags={tag.Replace(" ", "_")}";
            var webpage = await GetResponseStringAsync(url);
        var matches = Regex.Matches(webpage, "file_url=\"(?<url>.*?)\"");
            if (matches.Count == 0)
                throw new FileNotFoundException();
        ...
        /// System.Web.HttpUtility.UrlEncode(string url) need to make it post urls not unicode? %20 in link becomes space breaking link
